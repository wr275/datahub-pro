...
import { filesApi, calculatedFieldsApi } from '../api'
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      <div style={{ display: 'flex', borderRadius: 8, overflow: 'hidden', border: '1px solid #d1d5db', flexShrink: 0 }}>
        <button onClick={() => onTypeChange('column')} style={{ padding: '6px 10px', fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer', border: 'none', background: type === 'column' ? PINK : '#f8fafc', color: type === 'column' ? '#fff' : '#64748b' }} >Col</button>
        <button onClick={() => onTypeChange('constant')} style={{ padding: '6px 10px', fontSize: '0.78rem', fontWeight: 600, cursor: 'pointer', border: 'none', background: type === 'constant' ? PINK : '#f8fafc', color: type === 'constant' ? '#fff' : '#64748b' }} >123</button>
      </div>
      {type === 'column' ? (
        ...
        <input type="number" placeholder="0" value={const_ ?? ''} onChange={e => onConstChange(e.target.value === '' ? null : parseFloat(e.target.value))} style={{ ...INPUT, width: 100 }} />
      )}
    </div>
  )
}

function FieldRow({ field, columns, updateField, removeField, canRemove }) {
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap', padding: '12px 14px', borderRadius: 10, background: '#fdf2f8', border: '1px solid #fbc8e4', marginBottom: 10 }}>
      ...
      <input placeholder="New column name" value={field.name} onChange={e => updateField('name', e.target.value)} style={{ ...INPUT, width: 160 }} />
      <span style={{ color: '#94a3b8', fontSize: '0.8rem' }}>=</span>
      <OperandPicker type={field.col_a_type} col={field.col_a} const_={field.const_a} columns={columns} onTypeChange={v => updateField('col_a_type', v)} onColChange={v => updateField('col_a', v)} onConstChange={v => updateField('const_a', v)} />
      <select value={field.operator} onChange={e => updateField('operator', e.target.value)} style={{ ...SEL }}>
        {OPERATORS.map(op => <option key={op.value} value={op.value}>{op.label}</option>)}
      </select>
      <OperandPicker type={field.col_b_type} col={field.col_b} const_={field.const_b} columns={columns} onTypeChange={v => updateField('col_b_type', v)} onColChange={v => updateField('col_b', v)} onConstChange={v => updateField('const_b', v)} />
      {canRemove && (
...
export default function CalculatedFields() {
  ...
  const loadSets = () => {
    calculatedFieldsApi.list().then(r => setSavedSets(Array.isArray(r.data) ? r.data : [])).catch(() => {})
  }
    ...
    <div style={{ padding: '24px', maxWidth: 1100, margin: '0 auto' }}>
      ...
      <div style={CARD}>
        ...
        {fields.map((field, idx) => (
          <FieldRow key={idx} field={field} columns={allColumns(idx)} updateField={(k, v) => updateField(idx, k, v)} removeField={() => removeField(idx)} canRemove={fields.length > 1} />
        ))}
      ...
      <div style={CARD}>
        ...
        <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 20, flexWrap: 'wrap' }}>
          <input placeholder="Name this field set…" value={setName} onChange={e => setSetName(e.target.value)} style={{ ...INPUT, width: 260 }} />
          ...
          <>
            ...
            {savedSets.map(s => (
              <div key={s.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 14px', borderRadius: 8, background: '#f8fafc', border: '1px solid #e2e8f0', marginBottom: 8 }}>
                ...
                <span style={{ fontSize: '0.78rem', color: '#94a3b8' }}>{(s.fields || []).length} fields</span>
